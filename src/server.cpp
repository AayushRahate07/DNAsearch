    ...
    while (true)
    {
        ...
        else if (req.find("GET /search") == 0)
        {
            ...
            else
            {
                ...
                if (use_range)
                {
                    auto left = std::lower_bound(positions.begin(), positions.end(), lo);
                    auto right = std::upper_bound(positions.begin(), positions.end(), hi);

                    std::vector<size_t> filtered(left, right);
                    positions = filtered;
                }
